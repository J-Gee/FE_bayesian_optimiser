...
Adapted for Formulation Engine by Jack Gee
---Need to confirm it can read in the required headers and dispense data---
'''
...
chemspeed_flag = 1 # 0 is false, 1 is true
formengine_flag = 0

if formengine_flag == 1:
    from fe_parser import Parser

if chemspeed_flag == 1:
    from cs_parser import Parser


class Experiment:
    MINI_BATCH = 48
    BATCH = 15
    #BATCH_FILES = int(sys.argv[1])  # number of files that we want to see in the queue, should be BATCH/BATCH_FILES = MINI_BATCH
    BATCH_FILES = 1
    ...
    def queue_size(self):
        ...
        for f in os.listdir(folder_path):
            if os.path.isfile(os.path.join(folder_path, f)) and os.path.splitext(f)[1] == '.xlsx':
                if "opt" in os.path.splitext(f)[0]:
                    queue_size =+ 1

        return queue_size
                #
                #     book = load_workbook(folder_path+f)
                ...
                #     file_input = ws.cell(row=4, column=2).value
                #
                #
                #     if self.name in file_input:
                #         queue_size += 1
                #     book.close()
                # path = self.directory_path + '/runqueue/' + batch_name + '.xlsx'
                # book = load_workbook("fe_batch.xlsx")
                # # writer = pd.ExcelWriter("fe_batch.xlsx", engine="openpyxl")
                # worksheet_names = book.sheetnames
        ...
        for f in os.listdir(folder_path):
            ...
            if os.path.isfile(os.path.join(folder_path, f)) and os.path.splitext(f)[1] == '.xlsx':
                try:
                    ...
                    book = load_workbook(folder_path+f) #Basic intialisastion of worksheets into python so we can read off the exp name / #
                    worksheet_names = book.sheetnames
                    book.active = worksheet_names.index("Experiment Details")
                    ws = book.active
                    ...
                    if chemspeed_flag == 1:
                        if self.name in ws.cell(row=1, column=5).value and self.name + '-0' not in ws.cell(row=1, column=5).value:
                            clean = True



                    if formengine_flag == 1:
                        if self.name in ws.cell(row=4, column=2).value and self.name + '-0' not in ws.cell(row=4,
                                                                                                           column=2).value:
                            clean = True
    ...
    def skip_point(self, point):
        ...
        for key, value in point.iteritems():
            ...
            #taken from 5 umol to 500 umol to have this line ignored - meoh too likely to give unexpected o2 levels
            if key == 'calc_%_O2_umol' and value > 500:
                print('Warning, skipping leaky point ' + point['form_id'])
                return True

            '''         
               unedited case 1 in case errors
               # case 1: standard column that is not representing a compound
            #most of these are KUKA specific - only need water dispensed and h2 evol umol for optimisation
            ...
            # case 1: standard column that is not representing a compound
            # only need water dispensed and h2 evol umol for optimisation - rest stated here should be ignored by optimiser

            if (key in {'form_id', 'form_name', 'form_status', 'form_datetime', 'sample_name',
                        'calc_% H2_Avg', 'calc_%_O2_Avg', 'calc_%_H2_umol',
                        'calc_%_O2_umol', 'Water 1','internal_hydrogen_standard_micromol', "calc_%_N2_Avg", "calc_%_H2_2STD", "calc_%_H2_umol/h", "calc_%_O2_2STD", "calc_%_O2_umol/h", "calc_%_Ar_Avg", "calc_%_CO2_Avg"}) \
                    or 'Unnamed' in key:  # deal with faulty comma
                continue
    ...
    def update_points_and_targets(self):
        ...
        for filename in self.parser.process_completed_folder(self.name):
            ...
            # frame.dropna(subset=['calc_% H2_Avg'], #changed from "hyrogen_evolution"
            ...
            assert len(self.targets) == len(self.points), "Mismatch in points and targets. "\
    ...
    def optimisation_target(self, frame):
        return frame['calc_%_H2_umol'] # changed from kuka equiv
