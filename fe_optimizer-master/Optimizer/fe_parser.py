...
class Parser:
    ...
    def submit_mini_batch(self, batch_name, quantities_list, liquids):
        ...
        try:
            ...
            ws = book.active
            ws.cell(row=4, column=2).value = batch_name
            ...
            for i, quantity in enumerate(quantities_list):
                ...
                ws.cell(row=i+3, column=1).value = line[0] #puts batch_name into form id
                ...
                for n in range(len(line)-1):
                    ...
                    ws.cell(row=i + 3, column=col).value = headers[n+1]
                    col += 1
                    ws.cell(row=i + 3, column=col).value = line[n+1]
                    col += 1
                    if headers[n+1] in liquids:
